...
public class PokerstarsParserTest {

	/**
	 * LOGGER.
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(PokerstarsParserTest.class);
	...
	public void parsingSNGFile() {
		...
		Assert.assertEquals(new PokerstarsParser(null).stringToECards("8c"), flop[0]);
		Assert.assertEquals(new PokerstarsParser(null).stringToECards("8d"), flop[1]);
		Assert.assertEquals(new PokerstarsParser(null).stringToECards("Qd"), flop[2]);
	...
	public void testParseTableId() {
		String nextLine = "Table '943730146 1' 9-max Seat #4 is the button";
		final PokerstarsParser siteParsing = new PokerstarsParser(null);

		Assert.assertEquals("1", siteParsing.parseTableId(nextLine));
		nextLine = "Table '942746232 829' 9-max Seat #9 is the button";
		Assert.assertEquals("829", siteParsing.parseTableId(nextLine));
	}
	...
	public void testParseLevel() {
		...
		Assert.assertEquals(3, siteParsing.parseLevel(nextLine));
		...
		Assert.assertEquals(2, siteParsing.parseLevel(nextLine));
	...
	public void testParsePlayerSummary() {
		...
		nextLine = "Seat 9: misterwill8 (small blind) showed [Jd 4d] and won (200) with high card Ace";
		player = siteParsing.parsePlayerSummary(nextLine);
	...
	@Test
	public void testParsCurrency() {
		LOGGER.info("--- testParsCurrency() ---");

		String nextLine = "PokerStars Hand #121171794625: Tournament #966539218, €0.46+€0.04 EUR Hold'em No Limit - Match Round I, Level III (20/40) - 2014/09/07 4:28:28 CET [2014/09/06 22:28:28 ET]";
		final PokerstarsParser parser = new PokerstarsParser(null);
